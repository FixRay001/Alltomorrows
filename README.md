...
`run.bat` сам вызовет `build.bat`, если `.jar` ещё не собран.

Если Windows SmartScreen спросит подтверждение, выберите запуск файла, потому что
это локальный `.bat` из проекта.
